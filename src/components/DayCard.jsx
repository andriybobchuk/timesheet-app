    ...
    >
      <div className="flex flex-col items-center justify-center h-full">
        ...
        <div className="hidden sm:flex flex-col items-center justify-center h-full">
          ...
          {holiday && (
            <div className="absolute -top-1 -right-1 px-1.5 py-0.5 rounded-full bg-gradient-to-r from-purple-500 to-pink-500 shadow-lg">
          ...
          {hours > 0 && (
            <motion.div
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
              className="mt-0.5 px-1.5 py-0.5 rounded bg-gradient-to-r from-emerald-500/20 to-teal-500/20 border border-emerald-400/30"
            >
              <span className="text-[10px] font-bold text-emerald-300">{hours}h</span>
            </motion.div>
          )}
          
          {activity && (
            <div className="absolute -bottom-0.5 left-1/2 transform -translate-x-1/2 w-1.5 h-1.5 rounded-full bg-gradient-to-r from-violet-400 to-purple-400 animate-pulse"></div>
          )}
        </div>
      </div>
    </motion.div>
